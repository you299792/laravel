...
<head>
    ...
    <style>
        :root { --primary: #0f172a; --accent: #3b82f6; --accent-light: #60a5fa; --danger: #ef4444; --light: #f8fafc; --border: #e2e8f0; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-header { background: linear-gradient(135deg, var(--accent) 0%, var(--accent-light) 100%); color: white; padding: 2rem; display: flex; align-items: center; gap: 1rem; }
        .form-header i { font-size: 2rem; }
        .form-header h1 { margin: 0; font-size: 1.75rem; font-weight: 800; }
        .form-content { padding: 2.5rem; }
        .form-group { margin-bottom: 1.75rem; }
        label { display: block; margin-bottom: 0.75rem; font-weight: 700; color: var(--primary); font-size: 0.95rem; text-transform: uppercase; letter-spacing: 0.05em; }
        ...
        .error { color: var(--danger); font-size: 0.85rem; margin-top: 0.5rem; display: flex; align-items: center; gap: 0.35rem; }
        .button-group { display: flex; gap: 1rem; margin-top: 2.5rem; }
        .btn-primary { flex: 1; background: linear-gradient(135deg, var(--accent) 0%, var(--accent-light) 100%); color: white; padding: 1rem; border: none; border-radius: 0.75rem; font-weight: 700; font-size: 1rem; cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; gap: 0.75rem; box-shadow: 0 4px 15px rgba(59,130,246,0.2); }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(59,130,246,0.35); color: white; }
        ...
        .btn-secondary:hover { background: var(--border); transform: translateY(-2px); color: var(--primary); }
    </style>
</head>
<body>
    <nav class="navbar">
    ...
    <div class="container-wrapper">
        ...
        <div class="form-card">
            <div class="form-header">
                ...
                <div>
                    <h1>New User</h1>
                    <p style="margin:0; color:rgba(255,255,255,0.9);">Add a new user to your system</p>
                </div>
            </div>
            <div class="form-content">
                <form action="{{ route('users.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="email" name="email" placeholder="user@example.com" value="{{ old('email') }}" class="@error('email') is-invalid @enderror">
                        @error('email')<div class="error"><i class="fas fa-exclamation-circle"></i> {{ $message }}</div>@enderror
                    </div>
                    <div class="button-group">
                        <button type="submit" class="btn-primary"><i class="fas fa-check"></i> Create User</button>
